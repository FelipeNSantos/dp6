...
const DefineGA = function(hitType, eventCategory, eventAction, eventLabel) {
    const gaOBJ = { hitType, eventCategory, eventAction, eventLabel };
    ga('send', gaOBJ);
}

const EventGA = function(eventCategory, eventAction, eventLabel) {
    DefineGA('event', eventCategory, eventAction, eventLabel);
}
...
const OutsideDomainLink = function(link) {
    if (link == null || link === undefined) {
        return false;
    }
...
const getMenuEventName = function(el) {
...
const actMenuClassEvent = {
    ...
    inicio: () => {
        EventGA('menu', 'menu', 'inicio');
    },
    home: () => {
        EventGA('menu', 'menu', 'home');
    }
}
...
const clickLinkMenu = function(ev) {
    ...
    const menuClassEventName = getMenuEventName(el);
    ...
    } else if (OutsideDomainLink(link)) {
        eventAction = link;
    ...
    EventGA(eventCategory, eventAction, eventLabel);
}
